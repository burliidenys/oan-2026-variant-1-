...
import { AccessRequest } from '../shared/types';
interface AppState {
...
const API_URL = 'http://localhost:3000/api/v4/requests';
document.addEventListener('DOMContentLoaded', () => {
    const form = document.getElementById('accessRequestForm') as HTMLFormElement | null;
    fetchRequests();
...
function validateForm(userName: string, requestDate: string, details: string): boolean {
    clearErrors();
    let isValid = true;
...
async function handleFormSubmit(e: Event): Promise<void> {
    ...
    const requestDate = (document.getElementById('requestDate') as HTMLInputElement).value;
    const details = (document.getElementById('comments') as HTMLTextAreaElement).value.trim();
    const accessTypeId = (document.getElementById('accessType') as HTMLSelectElement).value;
    ...
    if (!userName) clientErrors.push("Ім'я користувача обов'язкове");
    else if (userName.length < 2) clientErrors.push("Ім'я занадто коротке (мінімум 2 символи)");
    
    if (!details) clientErrors.push("Деталі запиту обов'язкові");
    else if (details.length < 5) clientErrors.push("Деталі мають містити хоча б 5 символів");

    if (!accessTypeId) clientErrors.push("Будь ласка, оберіть тип доступу");
    if (clientErrors.length > 0 || !isFormValid) {
        alert("Помилка заповнення форми:\n• " + clientErrors.join("\n• "));
        return; 
    }
    if (!validateForm(userName, requestDate, details)) return;
...
(window as any).fetchRequestDetails = async (id: number) => {
    try {
...
(window as any).editEntry = (id: number) => {
    const entry = state.accessRequests.find(r => r.id == id);
    if (!entry) return;
    (document.getElementById('userName') as HTMLInputElement).value = entry.userName;
    (document.getElementById('comments') as HTMLTextAreaElement).value = entry.details;
    (document.getElementById('accessType') as HTMLSelectElement).value = entry.accessTypeId.toString() || "1";
    (document.getElementById('editId') as HTMLInputElement).value = entry.id.toString();
    const btn = document.querySelector('.btn-primary');
    if (btn) btn.textContent = "Оновити заявку";  
...
function fillForm(data: any): void {
    ...
    (document.getElementById('requestDate') as HTMLInputElement).value = data.requestDate;
    (document.getElementById('comments') as HTMLTextAreaElement).value = data.details;
    (document.getElementById('accessType') as HTMLSelectElement).value = data.accessTypeId;  
    const btn = document.querySelector('.btn-primary');
...
function resetForm(): void {
    ...
    const btn = document.querySelector('.btn-primary');
    if (btn) btn.textContent = "Зареєструвати заявку";
}
...
(window as any).deleteEntry = async (id: number) => {
